...
from okta_opa.main import execute_api_cycle, get_service_token


def test_get_service_token_success():
    """Test successful API call."""
    with patch("okta_opa.main.requests.post") as mock_post:
        mock_response = Mock()
...
def test_main_success():
    ...
    with patch.dict(
        ...
        with patch("okta_opa.main.get_service_token") as mock_get_token:
            with patch("okta_opa.main.get_resource_groups_by_team") as mock_get_rg:
                with patch(
                    "okta_opa.main.get_projects_by_resource_group"
                ) as mock_get_projects:
                    with patch(
                        "okta_opa.main.generate_server_enrollment_token"
                    ) as mock_gen_token:
...
def test_cli_module_structure():
    ...
    try:
        ...
        from okta_opa.main import execute_api_cycle as api_cycle
